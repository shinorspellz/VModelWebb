...
const Register = () => {
  ...
  // Redirect if userType or subCategory is missing
  useEffect(() => {
    if (!userType || !subCategory) {
      router.push('/user/getting-started');
    }
  }, [userType, subCategory, router]);
  ...
  return (
    <div className="flex flex-col lg:flex-row justify-center h-screen bg-white">
      ...
      <div className="lg:w-1/2 w-full flex-1 justify-center lg:h-auto h-full p-8">
        {success ? (
          <RegisterSuccess name={formData.firstName} />
        ) : (
          <RegisterForm
            formData={formData}
